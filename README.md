...
![UOF Webapp banner](assets/uof-arrive-banner.png)
...
- **Hosting elsewhere**: This is reiterated in the [Hosting it](#hosting-it) section, but if you plan on hosting the API there are two things to address:
      1. `frontend/index.html`'s `CLOUD_API` constant (currently `https://uof-webapp-api.onrender.com`, see [line 451](frontend/index.html#L451)) needs to point at your new host
...
4. **Point the frontend at the API's public URL** — Once `bridge.py` is reachable at a real URL, update the `CLOUD_API` constant in `frontend/index.html` (see [line 451](frontend/index.html#L451)) — the `BASE` constant just below it (line 452) falls back to this URL whenever it doesn't detect a local environment. This is the one line of application code you need to change to re-host this project somewhere new.
